...
int nuevoValorGen(vector< vector<int> >& matri,  int col, int fila, int jugador, vector< int> pesosPrioridad, vector<int> primDisp){ 
    int diagizq = pesosPrioridad[0]; // diagonalizqarriba, diagonalderarriba, vertical, horizontal
    int diagder = pesosPrioridad[1];
    int abajo = pesosPrioridad[2];
    int horiz = pesosPrioridad[3];
    int colaux = col;
    int filaaux = fila;
    int res = 0;

    /*if (rand() % 10 > 4){
        if (horizontalLibre( matri, primDisp, col, fila, jugador)){ // esta funcion sigue la estrategia de jugar en horizontal
            horiz = abs(horiz) + 100;
        }
    }*/
...
int aux;
    ...
    while (i < columns){
        aux = primDisp[i];
        if (aux < rows){
        	int auxiliar = nuevoValor(matri, i, primDisp[i], jugador);
            if ( auxiliar >= c){ // va de 0 a c
                return i;
            }
        }
    i++;
...
int greedy( vector< vector<int> >& matri, int jugador, vector<int> primDisp, vector<int>& pesosPrioridad){
	ofstream outputime;
    outputime.open ("tiempoej2.txt", std::ofstream::out | std::ofstream::app);
    int actual;
    int res;
    int max = -1000;
    int i = 0;
